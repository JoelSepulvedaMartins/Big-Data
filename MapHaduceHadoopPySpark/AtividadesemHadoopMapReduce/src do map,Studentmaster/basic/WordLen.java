...
public class WordLen {
    ...
    public static void main(String[] args) throws Exception {
        ...
        j.setMapperClass(MapForWordLen.class); // mapper
        j.setReducerClass(ReduceForWordLen.class); // reducer

        // Definição dos tipos de saída (map e reduce)
        j.setMapOutputKeyClass(IntWritable.class); // chave saída do map
        j.setMapOutputValueClass(IntWritable.class); // valor saída do map
        j.setOutputKeyClass(IntWritable.class); // chave saída do reduce
     ...
     */
    public static class MapForWordLen extends Mapper<LongWritable, Text, IntWritable, IntWritable> {
        /*
        ...
        public void map(LongWritable key, Text value, Context con)
            ...
            // gerando (chave, valor) para cada tamanho de palava
            for (String p: palavras){
                if (p.length() > 1){
                    IntWritable chaveSaida = new IntWritable(p.length()); //cast de int para IntWritable
    ...
    Tipo 4: tipo do valor de saída
     */
    public static class ReduceForWordLen extends Reducer<IntWritable, IntWritable, IntWritable, IntWritable> {
